...
// (média móvel desligada por padrão — MA_N=1)
constexpr int  MA_N = 1;                 // mude p/ 3 se quiser média móvel
...
const float Tt_tracking = 0.30f;   // s (0.2..0.8 típico)
const float I_LIM       = 1000.0f; // limite do integrador
const float T_leak      = 0.5f;    // s, "vazamento" do integrador parado

// ===== Velocidade plausível da MEDIÇÃO (p/ anti-spike dinâmico) =====
float vmax_mm_s = 150.0f; // ajuste prático (mm/s)
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      ...
      // ---- (LEGADO) fc= ignorado: filtro agora é mediana-3 + anti-spike dinâmico ----
      } else if (cmd.startsWith("fc=")) {
        Serial.println(F("INFO: fc= ignorado (usando mediana-3 + anti-spike dinamico)."));
      ...
      // ---- Velocidade plausível da medição (anti-spike) ----
  ...
  // Medidas: MEDIANA-3 + ANTI-SPIKE DINÂMICO (sem média móvel por padrão)
  for (int i = 0; i < 6; i++) {
    float v = analogMedian3Volts(FB_PINS[i]);         // 1) mediana de 3

    // 2) anti-spike dinâmico por física
    float max_dv_v = volts_per_mm(i) * vmax_mm_s * dt; // [V] por ciclo
    if (!guard_init) {
      guard_lastV[i] = v;
    } else {
    ...
    fbV_raw[i]  = v;
    // 3) média móvel (desligada com MA_N=1)
  ...
  if (manual_retract || manual_advance) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      // Dentro da deadband: solta atuador e faz "leak" do integrador
      if (fabs(e_mm) <= deadband_mm) {
        setDirFree(i);
        ledcWrite(PWM_CHANS[i], 0);
        last_pwm_cmd[i] = 0;

        // "vazamento" suave do integrador para zero
        if (Ki_mm[i] != 0.0f) {
      ...
      // ====== P, I (a ser atualizado depois), D ======
      float P = Kp_mm[i] * e_mm;
      float D = Kd_mm[i] * ydot_mmps;  // (vai subtrair)
      float I = Ki_mm[i] * integ[i];

      // Feedforward assimétrico com base no sinal do termo PID
      float pid_no_ff = P + I - D;
      float u_ff = (pid_no_ff >= 0.0f) ?  U0_adv[i] : -U0_ret[i];

      // Saída não-saturada (antes de PWM e MIN_PWM)
      float u_unsat = pid_no_ff + u_ff;

      // Saturação "física" do atuador (±MAX_PWM) — NÃO inclui MIN_PWM
      float u_sat = u_unsat;
      ...
      if (Ki_mm[i] != 0.0f) {
        ...
        // integra erro + correção de saturação
