...
// ===== Filtro mediana3 + EMA (IIR 1ª ordem) =====
float fc_hz = 4.0f;                   // ajustável por "fc="
bool  fb_init = false;
float fbV_raw[6];
float fbV_filt[6] = {0,0,0,0,0,0};
// >>> estados da mediana de 3 (janelas deslizantes por canal)
float prev1V[6] = {0,0,0,0,0,0};
float prev2V[6] = {0,0,0,0,0,0};
...
// >>> mediana de 3 (x0: novo, x1: anterior, x2: mais antigo)
inline float median3(float x0, float x1, float x2) {
  float a=x0,b=x1,c=x2;
...
void loop() {
  ...
  if (!fb_init) {
    for (int i = 0; i < 6; i++) {
      ...
      prev1V[i]   = fbV_raw[i];   // >>> inicia histórico da mediana
      prev2V[i]   = fbV_raw[i];
    }
    fb_init = true;
  }

  // >>> LPF com mediana3 + EMA:
  //     x_med = median3(x, x[-1], x[-2])
  //     y += alpha*(x_med - y),  alpha = dt/(RC+dt), RC=1/(2π fc)
  ...
  for (int i = 0; i < 6; i++) {
    float x     = fbV_raw[i];
    float x_med = median3(x, prev1V[i], prev2V[i]); // >>> mediana curta
    prev2V[i]   = prev1V[i];                        // >>> desliza janela
    prev1V[i]   = x;
    fbV_filt[i] += alpha * (x_med - fbV_filt[i]);   // >>> EMA
  }
