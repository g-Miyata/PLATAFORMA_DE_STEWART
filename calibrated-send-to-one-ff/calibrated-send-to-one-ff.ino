...
void loop() {
  ...
  if (manual_retract) {
    ...
    // --------- Controle P(+I) em mm + feedforward assimétrico ---------
    ...
    if (fabs(e_mm) > deadband_mm) {
      ...
      // P (+I)
      float uPI = Kp_mm[i]*e_mm + Ki_mm[i]*integ[i];

      // feedforward: só ajuda no sentido correspondente
      // (uPI >= 0) => SUBIR (positivo) -> soma +U0_adv
      // (uPI <  0) => DESCER (negativo) -> soma -U0_ret
      float u_ff = (uPI >= 0.0f) ?  U0_adv[i] : -U0_ret[i];
      float u    = uPI + u_ff;
