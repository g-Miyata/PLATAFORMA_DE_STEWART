...
async function openSerial() {
  ...
  try {
    ...
    document.getElementById('status-indicator').className = 'w-3 h-3 rounded-full bg-green-500 pulse-dot';
    document.getElementById('status-text').textContent = 'Conectado';
    document.getElementById('status-text').className = 'text-green-500 font-medium';
...
async function closeSerial() {
  try {
    ...
    document.getElementById('status-text').className = 'text-gray-400 font-medium';
...
function createActuator(startPos, endPos, actuator) {
  ...
  console.log(`  🔧 Criando atuador: start=${JSON.stringify(startPos)}, end=${JSON.stringify(endPos)}, length=${length.toFixed(1)}, valid=${actuator.valid}`);
...
function draw3DPlatform(containerId, data) {
  ...
  if (controls) {
    ...
    controls.update();
    console.log('📷 Câmera atualizada - target:', controls.target, 'position:', ctx.camera.position);
  }
...
function resetCamera(containerId) {
  ...
  console.log('✅ Câmera resetada - position:', camera.position, 'target:', controls?.target);
}
...
function updatePreviewMeasures(actuators) {
  ...
  (actuators || []).forEach((a, index) => {
    ...
    if (el && card) {
      ...
      console.log(`  P${index + 1}: length=${length.toFixed(1)}mm, valid=${isValid}`);
...
function updateMPUDisplay(mpu) {
  if (!mpu) return;
  const formattedRoll = mpu.roll.toFixed(1) > 10 ? 10 : mpu.roll.toFixed(1) < -10 ? -10 : mpu.roll.toFixed(1);
  const formattedPitch = mpu.pitch.toFixed(1) > 10 ? 10 : mpu.pitch.toFixed(1) < -10 ? -10 : mpu.pitch.toFixed(1);
  const formattedYaw = mpu.yaw.toFixed(1) > 10 ? 10 : mpu.yaw.toFixed(1) < -10 ? -10 : mpu.yaw.toFixed(1);
  ...
  document.getElementById('mpu-roll-bar').style.width = `${Math.max(0, Math.min(100, rollPercent))}%`;
  document.getElementById('mpu-pitch-bar').style.width = `${Math.max(0, Math.min(100, pitchPercent))}%`;
  document.getElementById('mpu-yaw-bar').style.width = `${Math.max(0, Math.min(100, yawPercent))}%`;
...
async function calculateKinematicsFromMPU(mpu) {
  try {
    ...
    const finalAngles = limitAngles(scaledAngles.roll, scaledAngles.pitch, scaledAngles.yaw);

    console.log('🎯 MPU Original:', { roll: mpu.roll.toFixed(2), pitch: mpu.pitch.toFixed(2), yaw: mpu.yaw.toFixed(2) });
    ...
    console.log('✅ Validação OK - Base:', data.base_points.length, 'Platform:', data.platform_points.length, 'Actuators:', data.actuators.length);
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    showToast('⚠️ Backend offline - verifique se FastAPI está rodando', 'warning');
  }
  ...
  // Calcular posição inicial neutra
  console.log('📐 Calculando posição inicial neutra...');
  const initialData = await calculateKinematicsFromMPU({ roll: 0, pitch: 0, yaw: 0 });

  if (initialData) {
    console.log('✅ Posição inicial calculada');
  } else {
    console.error('❌ Falha ao calcular posição inicial');
  }
  ...
  document.getElementById('btn-recalibrate').addEventListener('click', recalibrateMPU);
  ...
  window.testAPI = async () => {
    ...
    const testPose = { x: 0, y: 0, z: DEFAULT_Z_HEIGHT, roll: 0, pitch: 0, yaw: 0 };
    try {
      const resp = await fetch(`${API_BASE}/calculate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  document.getElementById('control-enabled').addEventListener('change', (e) => {
    ...
    if (controlEnabled) {
      showToast('Controle MPU ativado', 'success');
    } else {
