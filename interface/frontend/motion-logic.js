    ...
    try {
      ...
      transaction.oncomplete = () => {
        ...
        if (saveSuccesses % 10 === 0) {
          console.log(`💾 IndexedDB: ${saveSuccesses} registros salvos (transação completa)`);
        }
        resolve();
      };
...
async function flushDBWriteBuffer() {
  ...
  try {
    console.log(`💾 Gravando batch de ${batch.length} registros no IndexedDB...`);
...
function stopMotionGraph() {
  ...
  if (chartRecording && !isManualControl) {
    ...
    console.log('👤 Controle manual ativo - mantendo estado escolhido pelo usuário');
  }
}
        ...
        try {
          ...
          if (msg.type === 'motion_tick' && msg.pose_cmd) {
            ...
            if (msg.actuators && msg.actuators.length >= 6) {
              window.lastActuatorsReal = msg.actuators.map((a) => Number(a.length) || 0);
            } else if (msg.actuator_lengths_abs && msg.actuator_lengths_abs.length >= 6) {
              window.lastActuatorsReal = msg.actuator_lengths_abs.map((L) => Number(L) || 0);
            }
          }
        } catch (e) {
  ...
  const presetData = {
    sine_z: {
      ...
      params: [
        ...
        { label: 'Frequência (Hz)', name: 'hz', value: 0.5, min: 0.1, max: 1.5, step: 0.05 },
        { label: 'Duração (s)', name: 'duration_s', value: 45, min: 5, max: 300, step: 5 },
      ],
    },
    circle_xy: {
      color: 'purple',
      ...
      params: [
        ...
        { label: 'Raio Y (mm)', name: 'ay', value: 20, min: 10, max: 40, step: 1, grid: true },
        { label: 'Frequência (Hz)', name: 'hz', value: 0.75, min: 0.2, max: 1.5, step: 0.05 },
        { label: 'Duração (s)', name: 'duration_s', value: 60, min: 5, max: 300, step: 5 },
      ],
    },
    heave_pitch: {
      color: 'orange',
      ...
      params: [
        ...
        { label: 'Amplitude Pitch (°)', name: 'ay', value: 3.5, min: 1.5, max: 3.5, step: 0.5 },
        { label: 'Frequência (Hz)', name: 'hz', value: 0.8, min: 0.3, max: 0.8, step: 0.05 },
        { label: 'Duração (s)', name: 'duration_s', value: 40, min: 5, max: 300, step: 5 },
      ],
    },
    sine_pitch: {
      color: 'teal',
      ...
      params: [
        ...
        { label: 'Frequência (Hz)', name: 'hz', value: 0.8, min: 0.1, max: 0.8, step: 0.05 },
        { label: 'Duração (s)', name: 'duration_s', value: 30, min: 5, max: 300, step: 5 },
      ],
    },
    sine_roll: {
      color: 'indigo',
      ...
      params: [
        ...
        { label: 'Frequência (Hz)', name: 'hz', value: 0.8, min: 0.2, max: 0.8, step: 0.05 },
        { label: 'Duração (s)', name: 'duration_s', value: 30, min: 5, max: 300, step: 5 },
      ],
    },
    helix: {
      ...
      params: [
        ...
        { label: 'Raio Y', name: 'ay', value: 10, min: 10, max: 40, step: 1, grid: true },
        { label: 'Amp Z (mm)', name: 'z_amp_mm', value: 10, min: 10, max: 40, step: 0.5, grid: true },
        { label: 'Ciclos Z', name: 'z_cycles', value: 1, min: 0.2, max: 1, step: 0.5, grid: true },
        { label: 'Freq (Hz)', name: 'hz', value: 0.2, min: 0.1, max: 1.5, step: 0.05, grid: true },
        { label: 'Duração (s)', name: 'duration_s', value: 60, min: 5, max: 300, step: 5, grid: true },
      ],
    },
  };
