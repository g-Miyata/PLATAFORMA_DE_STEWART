...
- **Limites de Segurança**: ±10mm de translação e ±10° de rotação (configuráveis)
...
- **Horizontal (LX)**: Translação em X (±10mm)
- **Vertical (LY)**: Translação em Y (±10mm)

### Stick Direito

- **Horizontal (RX)**: Rotação Pitch (±10°)
- **Vertical (RY)**: Rotação Roll (±10°)

### Triggers (Futuro)

- **LT/RT**: Controle de Yaw (não implementado ainda)
...
#### Novo Endpoint: `POST /joystick/pose`
...
- **Translação X, Y**: ±10mm (configurável)
- **Rotação Roll, Pitch, Yaw**: ±10° (configurável)
- **Z**: Fixo em h0 (432mm) por padrão
...
2. **Tempo Real**: Taxa de atualização de 20Hz para controle suave
3. **Segurança**: Múltiplas camadas de validação e limites
4. **UX**: Preview 3D instantâneo, feedback visual claro
5. **Extensibilidade**: Fácil adicionar novos controles (ex: triggers para Yaw)
...
```python
MAX_TRANS_MM = 10.0   # Mudar para ±15mm
MAX_ANGLE_DEG = 10.0  # Mudar para ±15°
```
...
**Miyata** - Instituto Federal de São Paulo (IFSP)  
