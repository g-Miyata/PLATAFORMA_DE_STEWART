...
# GUI para controle completo (6 pistões) + telemetria + rampas
...
class App:
    def __init__(self, root):
        ...
        self.sel_var = tk.IntVar(value=1)   # pistão para modo manual e setagem de ganhos
        self.sp_global_var = tk.StringVar(value="0.0")
        self.sp_ind_vars = [tk.StringVar(value="0.0") for _ in range(6)]

        # ganhos por pistão
        self.kp_vars  = [tk.StringVar(value="1.0")  for _ in range(6)]
        self.ki_vars  = [tk.StringVar(value="0.00") for _ in range(6)]
        self.kd_vars  = [tk.StringVar(value="0.00") for _ in range(6)]

        # “para todos”
        self.kp_all_var  = tk.StringVar(value="1.0")
        self.ki_all_var  = tk.StringVar(value="0.00")
        self.kd_all_var  = tk.StringVar(value="0.00")
        self.u0a_all_var = tk.StringVar(value="10.0")
        self.u0r_all_var = tk.StringVar(value="10.0")
        ...
        # rampas (software)
        self.ramp_total_ms_var = tk.StringVar(value="2000")  # 2 s
        self.ramp_tick_ms_var  = tk.StringVar(value="50")    # 50 ms
        self._ramp_stop_evt = threading.Event()
        self._ramp_threads = []
    ...
    def _build_ui(self):
        ...
        # Console
        console = ttk.LabelFrame(mid, text="Console", padding=6)
        console.pack(side='left', fill='both', expand=True)
        self.txt = tk.Text(console, height=28, wrap='none')
        self.txt.pack(fill='both', expand=True, side='left')
        scroll = ttk.Scrollbar(console, command=self.txt.yview); scroll.pack(side='left', fill='y')
        ...
        # Setpoints + Rampas
        spbox = ttk.LabelFrame(right, text="Setpoints (mm) + Rampas", padding=6); spbox.pack(fill='x', pady=(8,0))
        ...
        r2 = ttk.Frame(spbox); r2.pack(fill='x', pady=2)
        ttk.Label(r2, text="Tempo total (ms):").pack(side='left')
        ttk.Entry(r2, textvariable=self.ramp_total_ms_var, width=8, justify='right').pack(side='left', padx=(4,10))
        ttk.Label(r2, text="Tick (ms):").pack(side='left')
        ttk.Entry(r2, textvariable=self.ramp_tick_ms_var, width=6, justify='right').pack(side='left', padx=(4,10))
        ttk.Button(r2, text="Rampa global", command=self.ramp_global).pack(side='left', padx=(6,0))
        ttk.Button(r2, text="Parar rampas", command=self.stop_ramps).pack(side='left', padx=(6,0))
        ...
        for i in range(6):
            ...
            ttk.Button(row, text="Rampa", command=lambda k=i: self.ramp_ind(k)).pack(side='left', padx=(6,0))
        ...
        # Ganhos/feedforward “para todos”
        allbox = ttk.LabelFrame(right, text="Aplicar em TODOS", padding=6); allbox.pack(fill='x', pady=(8,0))
        rga = ttk.Frame(allbox); rga.pack(fill='x', pady=2)
        ttk.Label(rga, text="Kp").pack(side='left')
        ttk.Entry(rga, textvariable=self.kp_all_var, width=6, justify='right').pack(side='left', padx=(2,8))
        ttk.Label(rga, text="Ki").pack(side='left')
        ttk.Entry(rga, textvariable=self.ki_all_var, width=6, justify='right').pack(side='left', padx=(2,8))
        ttk.Label(rga, text="Kd").pack(side='left')
        ttk.Entry(rga, textvariable=self.kd_all_var, width=6, justify='right').pack(side='left', padx=(2,8))
        ttk.Button(rga, text="Enviar TODOS", command=self.send_gains_all).pack(side='left', padx=(6,0))

        rff = ttk.Frame(allbox); rff.pack(fill='x', pady=2)
        ttk.Label(rff, text="U0_adv").pack(side='left')
        ttk.Entry(rff, textvariable=self.u0a_all_var, width=6, justify='right').pack(side='left', padx=(2,8))
        ttk.Label(rff, text="U0_ret").pack(side='left')
        ttk.Entry(rff, textvariable=self.u0r_all_var, width=6, justify='right').pack(side='left', padx=(2,8))
        ttk.Button(rff, text="Aplicar FF todos", command=self.send_ff_all).pack(side='left', padx=(6,0))
        ...
        ttk.Label(foot, text="Telemetria aceita: ms;SP_mm;Y1..Y6;PWM1..PWM6 (novo) ou ms;SP_mm;Y_mm;PWM (antigo).").pack(anchor='w')
    ...
    def connect(self):
        ...
        self._safe_log(f"[OK] Conectado em {port} @ {BAUD} bps\n")
        self._safe_log(f"[CSV] Gravando em: {self.csv_path}\n")
        ...
        # seleciona pistão atual para manual
        self.send_sel()
    ...
    def disconnect(self):
        ...
        if self.ser:
            ...
            self.ser = None
        self._close_csv()
        self._safe_log("[OK] Desconectado.\n")
    ...
    def _reader_loop(self):
        ...
        while not self.stop_flag.is_set():
            try:
                ...
                self._enqueue_rx(f"[ERRO leitura] {e}\n", False)
                break
        ...
        if buf:
            try:
                ...
                pass
        self._enqueue_rx("", False)  # marcador de fim

    def _enqueue_rx(self, s, to_csv):
        self.rx_queue.put((s, to_csv))

    def _tick_gui(self):
        try:
            while True:
                s, to_csv = self.rx_queue.get_nowait()
                if s == "":
                    break
                self._log(s)
                if to_csv:
                    self._write_csv("RX", s.rstrip("\n"))
                # parse telemetria
                self._try_parse_telem(s.strip())
        except queue.Empty:
            pass
        self.gui_timer = self.root.after(30, self._tick_gui)

    def _try_parse_telem(self, text):
        if not text or not text.startswith("ms;"):
            return
        parts = text.split(CSV_DELIM)
        try:
            # Novo: ms;SP_mm;Y1..Y6;PWM1..PWM6  => 1 + 1 + 6 + 6 = 14 campos
            if len(parts) >= 14:
                self.last_sp.set(self._fmt(parts[1]))
                for i in range(6):
            ...
            # Antigo: ms;SP_mm;Y_mm;PWM
                ...
                self.last_sp.set(self._fmt(parts[1]))
                self.y_vals[0].set(self._fmt(parts[2]))
                self.pwm_vals[0].set(self._fmt(parts[3], integer=True))
        except Exception:
            pass

    def _fmt(self, s, integer=False):
        try:
            x = float(str(s).replace(',', '.'))
            if integer:
    ...
    def _tx(self, s):
        ...
        try:
            ...
            self._safe_log(f"[ERRO TX] {e}\n")

    def _log(self, s):
        self.txt.insert('end', s)
        self.txt.see('end')

    def _safe_log(self, s):
        self._enqueue_rx(s, False)
    ...
    def send_gains_all(self):
        kp = self._parse_float(self.kp_all_var.get(), 0.0)
        ki = self._parse_float(self.ki_all_var.get(), 0.0)
        kd = self._parse_float(self.kd_all_var.get(), 0.0)
        self._tx(f"kpall={kp}")
        self._tx(f"kiall={ki}")
        self._tx(f"kdall={kd}")
    ...
    def send_ff_all(self):
        u0a = abs(self._parse_float(self.u0a_all_var.get(), 0.0))
        u0r = abs(self._parse_float(self.u0r_all_var.get(), 0.0))
    ...
    # ================== Rampas ==================
    def stop_ramps(self):
        self._ramp_stop_evt.set()
        for th in self._ramp_threads:
            try:
                th.join(timeout=0.3)
            except Exception:
                pass
        self._ramp_threads.clear()
        self._ramp_stop_evt.clear()
        self._safe_log("[RAMP] todas canceladas.\n")

    def ramp_global(self):
        target = self._parse_float(self.sp_global_var.get(), 0.0)
        total_ms = max(10, int(self._parse_float(self.ramp_total_ms_var.get(), 2000)))
        tick_ms  = max(5,  int(self._parse_float(self.ramp_tick_ms_var.get(), 50)))
        start = self._infer_start_sp(idx_hint=None)
        self._start_ramp_worker(kind="global", idx=None, start=start, target=target,
                                total_ms=total_ms, tick_ms=tick_ms)

    def ramp_ind(self, idx):
        target = self._parse_float(self.sp_ind_vars[idx].get(), 0.0)
        total_ms = max(10, int(self._parse_float(self.ramp_total_ms_var.get(), 2000)))
        tick_ms  = max(5,  int(self._parse_float(self.ramp_tick_ms_var.get(), 50)))
        start = self._infer_start_sp(idx_hint=idx)
        self._start_ramp_worker(kind="ind", idx=idx, start=start, target=target,
                                total_ms=total_ms, tick_ms=tick_ms)

    def _infer_start_sp(self, idx_hint=None):
        # tenta SP reportado; se não, tenta Y do idx; senão 0.0
        try:
            sp = float(str(self.last_sp.get()).replace(",", "."))
        except Exception:
            sp = None
        if sp is not None:
            return sp
        if idx_hint is not None:
            try:
                y = float(str(self.y_vals[idx_hint].get()).replace(",", "."))
                return y
            except Exception:
                pass
        return 0.0

    def _start_ramp_worker(self, kind, idx, start, target, total_ms, tick_ms):
        self._safe_log(f"[RAMP] start {kind} -> {target:.3f} mm | start={start:.3f} | total={total_ms}ms | tick={tick_ms}ms\n")
        stop_evt = self._ramp_stop_evt

        def work():
            try:
                steps = max(1, int((total_ms + tick_ms - 1) // tick_ms))
                dx = target - start
                for k in range(1, steps + 1):
                    if stop_evt.is_set() or self.stop_flag.is_set():
                        self._safe_log("[RAMP] cancelada.\n")
                        return
                    frac = k / float(steps)
                    sp = start + frac * dx
                    if kind == "global":
                        self._tx(f"spmm={sp:.3f}")
                    else:
                        self._tx(f"spmm{idx+1}={sp:.3f}")
                    time.sleep(tick_ms / 1000.0)
                # crava final
                if kind == "global":
                    self._tx(f"spmm={target:.3f}")
                else:
                    self._tx(f"spmm{idx+1}={target:.3f}")
                self._safe_log("[RAMP] concluída.\n")
            except Exception as e:
                self._safe_log(f"[RAMP] erro: {e}\n")

        th = threading.Thread(target=work, daemon=True)
        self._ramp_threads.append(th)
        th.start()
    ...
    def on_close(self):
        try:
            ...
            pass
        if self.gui_timer is not None:
            try: self.root.after_cancel(self.gui_timer)
            except Exception: pass
        self.root.destroy()
        try:
            sys.exit(0)
