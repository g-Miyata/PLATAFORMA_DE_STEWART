...
# - NOVO: Rampa (por software) global e por pistão
#
# Observação:
...
class App:
    def __init__(self, root):
        ...
        self._ramp_threads = []   # para gerenciar cancelamento
    ...
    def _build_ui(self):
        ...
        # ===== NOVO: parâmetros da rampa global =====
        ...
        ttk.Button(r2, text="Rampa em todos", command=self.ramp_global).pack(side='left', padx=(6,0))
        ttk.Button(r2, text="Parar rampa", command=self.stop_ramps).pack(side='left', padx=(6,0))
        ...
        ttk.Label(foot, text="Dica: use spmm=VAL para global e spmmN=VAL para individual. Para rampa, preencha os tempos e clique nos botões.").pack(anchor='w')
    ...
    def disconnect(self):
        self.stop_ramps()  # garante cancelar rampas
        self.stop_flag.set()
    ...
    def _on_rx_line(self, text):
        ...
        try:
            if text.startswith("ms;"):  # tenta parsear as duas versões
                ...
                # Novo formato: ms;SP_mm;Y1..Y6;PWM1..PWM6
                if len(parts) >= 14:
                    sp = parts[1]
                    self.last_sp.set(sp)
                    for i in range(6):
    ...
    def _val_or_none(self, var_str):
        """tenta converter uma tk.StringVar (com '--' etc.) para float; retorna None se não der."""
        try:
            v = float(str(var_str.get()).replace(",", "."))
            if v != v:  # NaN
                return None
            return v
        except Exception:
    ...
    # Feedforward (placeholders se quiser completar depois)
    ...
    # ================== RAMPAS (por software) ==================
    def stop_ramps(self):
        # sinaliza cancelamento e espera threads
        ...
        self._log("[RAMP] todas canceladas.\n")
    ...
    def ramp_global(self):
        ...
        # ponto inicial: tenta SP atual; se não, tenta Y1; se não, 0.0
        sp_now = self._val_or_none(self.last_sp)
        if sp_now is None:
            sp_now = self._val_or_none(self.y_vals[0])
        if sp_now is None:
            sp_now = 0.0

        self._start_ramp_worker(kind="global", idx=None, start=sp_now, target=target,
    ...
    def ramp_ind(self, idx):
        ...
        # ponto inicial: tenta SP atual (é único, mas serve como estimativa)
        sp_now = self._val_or_none(self.last_sp)
        # se não houver, tenta telemetria desse eixo
        if sp_now is None:
            sp_now = self._val_or_none(self.y_vals[idx])
        if sp_now is None:
            sp_now = 0.0

        self._start_ramp_worker(kind="ind", idx=idx, start=sp_now, target=target,
                                total_ms=total_ms, tick_ms=tick_ms)

    def _start_ramp_worker(self, kind, idx, start, target, total_ms, tick_ms):
        """kind: 'global' ou 'ind'; idx é ignorado no global."""
        self._log(f"[RAMP] start {kind} -> {target:.3f} mm | start={start:.3f} | total={total_ms}ms | tick={tick_ms}ms\n")
        ...
        def work():
            try:
                ...
                # garante último ponto cravado
                if kind == "global":
                    self._tx(f"spmm={target:.3f}")
                else:
                    self._tx(f"spmm{idx+1}={target:.3f}")
                self._log("[RAMP] concluída.\n")
            except Exception as e:
                self._log(f"[RAMP] erro: {e}\n")
